...
public class BlockAttractor extends ModBlock implements IConvertible {
    ...
    public BlockAttractor() {
        super(LibBlocks.ATTRACTOR, 1.5F, 10.F, Material.ROCK);
    ...
    public IBlockState getActualState(@Nonnull IBlockState state, IBlockAccess world, BlockPos pos) {
        ...
        if (tile == null || !(tile instanceof TileEntityAttractor)) return super.getActualState(state, world, pos);
        TileEntityAttractor tileAttractor = (TileEntityAttractor) tile;
        return state.withProperty(ON_STATE, tileAttractor.isOn()).withProperty(INVERTED, tileAttractor.isInverted());
    }
    ...
    public TileEntity createTileEntity(@Nonnull World world, @Nonnull IBlockState state) {
        return new TileEntityAttractor();
    }
    ...
    public void convertState(World world, BlockPos pos, EntityPlayer player) {
        TileEntity tile = world.getTileEntity(pos);
        if (!(tile instanceof TileEntityAttractor)) return;
        boolean inverted = ((TileEntityAttractor) tile).isInverted();

        world.setBlockState(pos, ModBlocks.Impeller.getDefaultState());

        tile = world.getTileEntity(pos);
        if (tile instanceof TileEntityImpeller) {
            ((TileEntityImpeller) tile).setInverted(inverted);
        }
    }
